...
def critique_glm(posterior_pred, x, x_test, y_test, w, b, qw, qb):
    t_max = lambda data_dict, latent: tf.reduce_max(data_dict[posterior_pred])
    t_min = lambda data_dict, latent: tf.reduce_min(data_dict[posterior_pred])
    t_mean = lambda data_dict, latent: tf.reduce_mean(data_dict[posterior_pred])
    ppc_1 = ed.ppc(t_max,
                   data={
                       x: x_test.as_matrix(),
